...
def optimize_policy(
    ...
    es = cma.CMAEvolutionStrategy(initial_solution, sigma, {'popsize': popsize, 'maxiter': max_gens})
    
    best_rewards = []
    while not es.stop():
        solutions = es.ask()
        rewards = [evaluate_weights(s, env_name, episodes_per_eval) for s in solutions]
        es.tell(solutions, [-r for r in rewards]) 
        best_rewards.append(max(rewards))
    
    return best_rewards, es.result.xbest
